  ...
  try {
    const response = await axios.get('/api/designers', {
      params: {
        ...
        city: city.value,
        sort: sort.value, // tambahkan sort ke params
      }
    });
...
// Hanya satu onMounted, panggil fetchDesigners
onMounted(() => {
  fetchDesigners();
});
</script>
  ...
  <div class="flex items-center bg-white rounded-[20px] h-12 px-4 shadow-sm w-full max-w-[1000px] relative">
    ...
    <div class="w-full max-w-[1100px] mx-auto px-8 pb-12 relative">
      ...
      <div v-else class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <img
            :src="designer.avatar_url || '/images/default_Avatar.png'"
            :alt="designer.name"
          ...
          <div class="p-4 text-center">
            ...
            <div class="flex flex-wrap justify-center gap-2 mb-3">
              ...
              <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-[#DBEAFE] text-blue-900">
                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M3 21V6a1 1 0 011-1h5v2H5v14h14V7h-4V5h5a1 1 0 011 1v15H3z"/>
                  <path stroke-linecap="round" stroke-linejoin="round" d="M9 21V10h6v11"/>
                </svg>
                {{ designer.origin_city }}
              </span>
            </div>
          </div>
        </Link>
